            ...
            @foreach ($results as $team)
                <div class="flex-col">
                    ...
                    @foreach ($team as $player)
                        <div class="flex justify-between border-double border-2">
                            <h class="my-4 mx-9 text-zinc-100 text-2xl">{{ $player->player_name }}</h>
                            <h class="my-4 mx-9 text-zinc-100 text-2xl">{{ $player->position_pretty }}</h>
                            <h class="my-4 mx-9 text-zinc-100 text-2xl">{{ number_format($player->smvp, 1) }}</h>
                        </div>
                    @endforeach
                </div>
            @endforeach
