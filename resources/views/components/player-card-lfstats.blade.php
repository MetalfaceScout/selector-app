...
>
    <div class="flex justify-between items-start">
        <div class="px-2">
            <p wire:key="player-card-{{ $player['player_id'] }}-lfstats" @class([
                'font-medium whitespace-nowrap text-zinc-800 dark:text-zinc-200'
            ])>{{ $player['player_name'] }}</p>
        </div>

        <div x-data="{ 
            open: false, 
            commander_mvp: {{ $player['commander_avg_mvp'] }}, 
            heavy_mvp: {{ $player['heavy_avg_mvp'] }},
            scout_mvp: {{ $player['scout_avg_mvp'] }},
            ammo_mvp: {{ $player['ammo_avg_mvp'] }},
            medic_mvp: {{ $player['medic_avg_mvp'] }},
            lfstats_id: {{ $player['player_id'] }},
            }"
            class="relative"
            >
...
"
            <x-primary-button @click.="$wire.addPlayerFromLfstats( {{ $player['player_id'] }} )">Add</x-primary-button>
