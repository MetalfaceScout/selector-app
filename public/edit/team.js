...
class Player {
    ...
    /* getAvgByName(idname) {
        ...
        for (let i = 0; i < POSITIONS_IDS.length; i++) {
            if (POSITIONS_IDS[i] == idname) {
                indexToArray = i;
            }
        }
        if (indexToArray == -1) {
            console.error("TeamObject: getAvgByName: Not given correct ID name")
            return
        }
        return this.avgMVPArray[indexToArray];
    }

    setAvgByName(idname, value) {
        let indexToArray = -1
        for (let i = 0; i < POSITIONS_IDS.length; i++) {
            if (POSITIONS_IDS[i] == idname) {
                indexToArray = i;
        ...
        if (indexToArray == -1) {
            console.error("TeamObject: setAvgByName: Not given correct ID name")
            return
        }
        this.avgMVPArray[indexToArray] = value;
    } */

    setAvgByPosition(position, avg) {
        this[position.toLowerCase() + "_avg_mvp"] = avg;
    }

    setAvgAvg(avg) {
        if (!isNaN(Number(avg))) {
            this["avg_mvp"] = avg;
            console.log("set avg to " + avg);
        }        
    }
}
...
export {Team, Player, createTeamRandom, teamFormats, POSITIONS_IDS}